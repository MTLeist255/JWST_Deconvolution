...
# set import paths
# set the import path for the RL deconvolved image
path = 'Images/JWST/4_Deconvolution/'+str(Dir)+'/FITS/' + str(model_type) + '/' + str(model) + '/21new/'
# set the output path for the merit function measurements
out_path = 'Images/JWST/4_Deconvolution/'+str(Dir)+'/Measurements/' + str(model_type) + '/' + str(model)

# read in deconvolved data and set the pixel scale and observation wavelength
decon_ims = get_pkg_data_filename(path + 'SCALED_' + str(filter) + '_' + str(model) + '_' + str(sample) + '.fits')
...
with open(out_path + '/' + str(filter) + '/21new/RL_SELECT_' + str(model) + '_' +str(sample) + '_SCALED.txt','w') as f:
     f.write('\n'.join(['\n'.join(lines) for lines in lineA]) + '\n')

# save total measurements to a single text file
with open(out_path + '/' + str(filter) + '/21new/RL_TOTAL_' + str(model) + '_' +str(sample) + '_SCALED.txt','w') as f:
     f.write('\n'.join(['\n'.join(lines) for lines in lineB]) + '\n')

#save the merit functions
data2 = ''
with open (out_path + '/' + str(filter) + '/21new/RL_Image_Plot_' + str(model) + '_' +str(sample) + '_SCALED.txt', 'w') as fh:
   for a, b, c in zip(merit_nint, merit_flux, merit_fwhm):
       print('%.0f  %.5f  %.5f ' % (a, b, c), file = fh)

with open(out_path + '/' + str(filter) + '/21new/RL_Image_Plot_' + str(model) + '_' +str(sample) + '_SCALED.txt', 'r') as fp:
   data2 = fp.read()

with open(out_path + '/' + str(filter) + '/21new/RL_Image_Plot_' + str(model) + '_' +str(sample) + '_SCALED.txt', 'w') as fp:
   fp.write(data2)
...
if photom_test:
    ...
    # save the merit functions
    data2 = ''
    with open(out_path + '/' + str(filter) + '/21new/Photometry_Tests/SCALED_PHOTOM_Testing_' + str(model) + '_' +str(sample) + '.txt', 'w') as fh:
    ...
    with open(out_path + '/' + str(filter) + '/21new/Photometry_Tests/SCALED_PHOTOM_Testing_' + str(model) + '_' +str(sample) + '.txt', 'r') as fp:
        data2 = fp.read()

    with open(out_path + '/' + str(filter) + '/21new/Photometry_Tests/SCALED_PHOTOM_Testing_' + str(model) + '_' +str(sample) + '.txt', 'w') as fp:
        fp.write(data2)
